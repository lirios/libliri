...
QObjectListModel::QObjectListModel(const std::function<QObject *()> &factory, QObject *parent)
...
int QObjectListModel::rowCount(const QModelIndex &) const { return m_data.count(); }
...
QVariant QObjectListModel::get(int index) const { return QVariant::fromValue(m_data.at(index)); }
...
bool QObjectListModel::removeFirst() { return removeAt(0); }

bool QObjectListModel::removeLast() { return removeAt(m_data.count() - 1); }
...
bool QObjectListModel::moveRows(const QModelIndex &sourceParent, int sourceRow, int count,
...
{
    if (ev->timerId() != m_notifyTimer.timerId())
        return;
    Q_EMIT dataChanged(index(m_notifyIndexes.begin().key()),
                       index((m_notifyIndexes.end() - 1).key()), QVector<int>(1, Qt::DisplayRole));
    m_notifyTimer.stop();
    ...
    if (m_tracking)
        for (int i = 0; i < props; ++i) {
            ...
            if (prop.hasNotifySignal())
                connect(obj, prop.notifySignal(), this, nSlot);
        }
    else {
