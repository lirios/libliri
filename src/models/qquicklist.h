...
public:
    ...
    explicit inline QQuickList(const QList<T *> &list)
            : QList<T *>(list), m_model(QObjectListModel::create(list))
    {
    }
    ...
    inline QObjectListModel *getModel()
    {
        return m_model;
    }
    ...
    inline QQuickList(QList<T *> &&other)
            : QList<T *>(other), m_model(QObjectListModel::create(other))
    {
        T *assertObject = new T();
        Q_ASSERT_X(qobject_cast<QObject *>(assertObject), "QQuickList<T>::QQuickList",
                   "Typename T does not inherit from QObject.");
        delete assertObject;
    }
    ...
    inline QQuickList(std::initializer_list<T> args) : QList<T *>(args)
    {
    }
#endif
    ...
    inline void append(const QList<T *> &t)
    {
        QList<T *>::append(t);
        m_model->insert(t);
    }
    inline void prepend(T *const &t)
    {
        QList<T *>::prepend(t);
        m_model->insert(t, 0);
    }
    inline void insert(int i, T *const &t)
    {
        QList<T *>::insert(i, t);
        m_model->insert(t, i);
    }
    inline void replace(int i, T *const &t)
    {
        QList<T *>::replace(i, t);
        m_model->replace(t, i);
    }
    ...
    inline int removeAll(T *const &t)
    {
        m_model->removeAll(t);
        return QList<T *>::removeAll(t);
    }
    inline bool removeOne(T *const &t)
    {
        m_model->removeOne(t);
    ...
    inline QQuickList<T> &operator+(const QList<T *> &l)
            const // You should not add elements like this (for fluidity purpose)
    {
        QQuickList<T> *n = new QQuickList<T>();
        n->append(*this);
        n->append(l);
        return *n;
    }
    ...
    template <typename LessThan> inline void sort(LessThan lessThan)
    {
...
template <typename T> inline typename QQuickList<T>::iterator QQuickList<T>::erase(iterator pos)
{
